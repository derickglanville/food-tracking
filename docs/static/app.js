...
function nutritionSummary(entries){const calories=entries.reduce((sum,entry)=>sum+nutritionValue(entry,'calories'),0),salt=entries.reduce((sum,entry)=>sum+nutritionValue(entry,'saltGrams'),0),sugar=entries.reduce((sum,entry)=>sum+nutritionValue(entry,'sugarGrams'),0);const saltRemaining=Math.max(0,2-salt),sugarRemaining=Math.max(0,29-sugar),calorieRemaining=Math.max(0,dailyCalorieGoal-calories);const advice=[];if(calories>=dailyCalorieGoal)advice.push(`Your ${dailyCalorieGoal.toLocaleString()}-calorie goal has been reached — choose a lighter option for the rest of today.`);else advice.push(`${calorieRemaining.toLocaleString()} calories remaining toward your ${dailyCalorieGoal.toLocaleString()}-calorie daily goal.`);if(salt>=2)advice.push('Salt limit reached — choose fresh foods and avoid added salt for the rest of today.');else advice.push(`${saltRemaining.toFixed(1)} g of salt remaining today.`);if(sugar>=29)advice.push('Sugar limit reached — choose unsweetened drinks and foods with no added sugar.');else advice.push(`${sugarRemaining.toFixed(0)} g of added sugar remaining today.`);if(!entries.some(entry=>entry.calories||entry.saltGrams||entry.sugarGrams))advice.push('Add estimates to meals to see daily nutrition guidance.');return `<section class="nutrition-summary" aria-label="Daily nutrition estimates"><div><b>≈ ${calories.toLocaleString()} of ${dailyCalorieGoal.toLocaleString()} calories</b><span>${salt.toFixed(1)} g salt · ${sugar.toFixed(0)} g added sugar</span></div><p>${advice.join(' ')}</p><small>Estimates are for meal planning; check labels or a nutrition professional for precise guidance.</small></section>`;}
...
function row(entry) {
  ...
  const nutrition=[entry.calories&&`≈ ${escapeHtml(entry.calories)} cal`,entry.saltGrams&&`${escapeHtml(entry.saltGrams)} g salt`,entry.sugarGrams&&`${escapeHtml(entry.sugarGrams)} g sugar`].filter(Boolean).join(' · ');
