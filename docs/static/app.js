...
const nutritionValue=(entry,field)=>{const value=Number(entry[field]);return Number.isFinite(value)&&value>=0?value:0;};
function nutritionSummary(entries){const derickEntries=entries.filter(entry=>String(entry.derickMeal||'').trim());const calories=derickEntries.reduce((sum,entry)=>sum+nutritionValue(entry,'calories'),0),salt=derickEntries.reduce((sum,entry)=>sum+nutritionValue(entry,'saltGrams'),0),sugar=derickEntries.reduce((sum,entry)=>sum+nutritionValue(entry,'sugarGrams'),0);const saltRemaining=Math.max(0,2-salt),sugarRemaining=Math.max(0,29-sugar),calorieRemaining=Math.max(0,dailyCalorieGoal-calories);const advice=[];if(calories>=dailyCalorieGoal)advice.push(`Your ${dailyCalorieGoal.toLocaleString()}-calorie goal has been reached — choose a lighter option for the rest of today.`);else advice.push(`${calorieRemaining.toLocaleString()} calories remaining toward your ${dailyCalorieGoal.toLocaleString()}-calorie daily goal.`);if(salt>=2)advice.push('Salt limit reached — choose fresh foods and avoid added salt for the rest of today.');else advice.push(`${saltRemaining.toFixed(1)} g of salt remaining today.`);if(sugar>=29)advice.push('Sugar limit reached — choose unsweetened drinks and foods with no added sugar.');else advice.push(`${sugarRemaining.toFixed(0)} g of added sugar remaining today.`);if(!derickEntries.some(entry=>entry.calories||entry.saltGrams||entry.sugarGrams))advice.push('Add estimates to Derick’s meals to see daily nutrition guidance.');return `<section class="nutrition-summary" aria-label="Derick daily nutrition estimates"><div><b>≈ ${calories.toLocaleString()} of ${dailyCalorieGoal.toLocaleString()} calories</b><span>${salt.toFixed(1)} g salt · ${sugar.toFixed(0)} g added sugar</span></div><p>${advice.join(' ')}</p><small>Derick’s breakfast, lunch, dinner, and snack estimates only. Check labels or a nutrition professional for precise guidance.</small></section>`;}
...
function row(entry) {
  ...
  const nutrition=entry.derickMeal?[entry.calories&&`Derick: ≈ ${escapeHtml(entry.calories)} cal`,entry.saltGrams&&`${escapeHtml(entry.saltGrams)} g salt`,entry.sugarGrams&&`${escapeHtml(entry.sugarGrams)} g sugar`].filter(Boolean).join(' · '):'';
  return `<article class="meal-row"><span class="meal-icon ${escapeHtml(entry.mealType)}" aria-hidden="true">${icons[entry.mealType]||'✧'}</span><div><div class="meal-title">${escapeHtml(main)}</div>${other?`<div class="meal-other">${escapeHtml(other)}</div>`:''}<div class="meal-meta"><span>${formatDate(entry.date)}</span><span>${escapeHtml(entry.mealType)}</span><span>${escapeHtml(entry.preparer||'Preparer not recorded')}</span>${nutrition?`<span>${nutrition}</span>`:''}${entry.needsReview?'<span class="review-badge">Review preparer</span>':''}</div></div><button class="text-button" data-edit="${escapeHtml(entry.id)}" aria-label="Edit ${escapeHtml(entry.mealType)} on ${escapeHtml(entry.date)}">Edit ↗</button></article>`;
}
...
function dayCard(){
  ...
  ${nutritionSummary(entries)}<div class="day-meals">${types.map(type=>{
...
function openMeal(entry={}) {
  ...
  const defaults={id:'',date:today(),mealType:'Dinner',meal:'',derickMeal:'',preparer:'',preparation:'Unspecified',calories:'',saltGrams:'',sugarGrams:'',notes:'',source:'Manual entry',...entry};
